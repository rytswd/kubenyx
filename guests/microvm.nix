...
in
{
  ...
  # IPAddressAllow, the clockstep poke filter, and the launcher CA channel.
  # The default is the default mesh subnet's gateway, so existing configs
  # render byte-identically; lib/microvm.nix's mkNode sets it from the
  # cluster subnet. The SLiRP gateway (qemu variant) stays a separate
  # constant — user-mode networking never moves.
  options.kubenyx.hostGateway = lib.mkOption {
    type = lib.types.str;
    default = "10.100.0.1";
    description = "Host-side gateway address on the guest subnet, trusted for host↔guest handoffs.";
  };
        ...
        lib.optionalAttrs isServer {
          kubenyx-kubeconfig = {
            ...
            socketConfig = {
              Accept = true;
              IPAddressAllow = [
                "${cfg.hostGateway}/32" # tap gateway (firecracker / cloud-hypervisor)
      ...
      systemd.services =
        ...
        // {
          ...
          kubenyx-clockstep = {
            ...
            serviceConfig = {
              ExecStart = "${lib.getExe' cfg.internal.tools "kubenyx-clockstep"} --allow-from ${cfg.hostGateway}";
              Restart = "always";
              RestartSec = 1;
            };
          };
